...
class SimulationsController extends AppController {
	...
	public function calc($penetration)	{
		...
		} elseif (array_key_exists('defaultValues', $this->request->data['Simulations']) !== false) {
			$defaultValues = unserialize($this->request->data['Simulations']['defaultValues']);
		} else {
			$defaultValues = null;
		}
		...
		if (!empty($this->request->data) && array_key_exists('defaultValues', $this->request->data['Simulations']) === false)	{
			...
			foreach($this->distris[$penetration] as $key => $distri)	{
				$bp = 0.0; 
				if (array_key_exists($key, $this->request->data["Simulations"]) === false)	{
					if ($key < self::MIN_SPREAD)
						$bp = floatval($this->request->data["Simulations"][self::MIN_SPREAD]);
					else
						$bp = floatval($this->request->data["Simulations"][self::MAX_SPREAD]);
				} else
					$bp = floatval($this->request->data["Simulations"][$key]);
