...
class ToolsController extends AppController {
	...
	public function Calc()	{
		...
		if (!empty($this->request->data))	{
			...
			foreach ($this->distris as $key => $distri)	{
				$ev += $distri[0] * $distri[1] * $units[$this->request->data['Tools'][$key]] / 100.0;
				$sd += pow($units[$this->request->data['Tools'][$key]], 2) * $distri[0];  
			}
			$sd = 1.15 * sqrt($sd);
	...
	public function Estimation()	{
		if (!empty($this->request->data))	{
			$ev = $this->request->data['Tools']['ev'];
			$ev = $this->request->data['Tools']['ev'];
			$sd = $this->request->data['Tools']['sd'];
			$unit = $this->request->data['Tools']['unit'];
			$hours = $this->request->data['Tools']['hours'];
			$ev *= $unit * $hours;
			$sd *= $unit * sqrt($hours);
			$min = $ev - $sd;
			$max = $ev + $sd; 
	...
	public function EstimationStrategieBase()	{
		if (!empty($this->request->data))	{
			$advantage = $this->request->data['Tools']['advantage'];
			$unit = $this->request->data['Tools']['unit'];
			$hours = $this->request->data['Tools']['hours'];
